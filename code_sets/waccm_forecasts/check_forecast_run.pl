...
$plotlog = "/glade/u/home/buchholz/code_database/code_sets/waccm_forecasts/.waccm_plotlog";
open(IN, "<$plotlog");
chomp(@lines = <IN>);
close(IN);
...
open(OUT,">/glade/u/home/buchholz/code_database/code_sets/waccm_forecasts/temp.out");
...
if ($end_file ne '' && $plotted == 0){
  ...
  `sbatch /glade/u/home/buchholz/code_database/code_sets/waccm_forecasts/submit_script_slurm`;
  open(OUT2,">>$plotlog");
...
if (($time == 18 || $time == 23)&& $plotted == 0){
  ...
  $subject = 'WACCM forecast plotting not done';
  $message = 'After 6pm, WACCM forecast plotting not done for '. $current_date .', may need to manually plot.';
...
if ($time == 22 && $plotted == 1){
  `rm -f /glade/u/home/buchholz/code_database/code_sets/waccm_forecasts/forecast_plots/WACCM*.png`;
}
